...
with conn:
    ...
    bar_ax.set_xticklabels(mers_class, rotation=45, ha='right')
    bar_ax.set_ylabel('Ціна, €')
    bar_ax.set_title('Ціни на мерседеси')
    for i, label in enumerate(mers_class):
        bar_ax.text(i, 15, label, ha='left', va='bottom', fontsize=10)
    figure.text(0.5, 0.02, 'Клас мерседесів', ha='center', va='center', fontsize=10)
